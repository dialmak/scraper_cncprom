// Довідка як меню-дропдаун: «Навіщо це», «Що означають цифри та позначки»
// (панель кожної сторінки своя) і «Про проєкт і подяки». Живе тут, а не в
// кожному зі скриптів-генераторів, бо однакове на трьох сторінках — map.html,
// мапі розділу й історії змін; три копії розійшлись би.
//
...
const helpMenuHtml = (termsTip) => `
      <div class="hdr-menu">
        ...
        <div class="hdr-dropdown right" id="help-dropdown">
          ...
          <div>${menuRow(`<span class="ico-green">${ICONS.about}</span>`, 'Навіщо це', 'about-overlay', 'Що це за проєкт і як він працює')}${menuRow(ICONS.legend, 'Що означають цифри та позначки', 'help-overlay', termsTip)}${menuRow(ICONS.credits, 'Про проєкт і подяки', 'credits-overlay', 'Хто зробив і на чому все це зроблено')}
          </div>
        </div>
      </div>`;
...
const CREDITS = [
  ...
  { logo: 'nicua.png', name: 'nic.ua', url: 'https://nic.ua/',
    desc: 'безкоштовний домен cncprom.pp.ua, на якому живе цей сайт' },
  ...
  <div class="help-overlay" id="credits-overlay">
    <div class="help-panel">
      ...
      <div class="help-panel-body">
        ...
        <p class="failed-note">Велика подяка розробникам використаних компонентів та технологій:</p>
...
// двох десятків HTML. Переважно SVG; nic.ua віддає свій знак лише растром
// (їхній SVG — широкий логотип-напис 98×27, в квадраті 30×30 він став би нечитним),
// тож там PNG 152×152 — його ж apple-touch-icon.
...
// Пункт «Пошук» у панелі пояснень — однаковий на map.html і мапі розділу, бо
// пошук той самий (map-common.js). Мапа розділу додає речення про охоплення.
const searchHelpHtml = (extra = '') => `
        <div class="help-term">
          <div class="help-term-label">${ICONS.search} Пошук</div>
          <div class="help-term-desc">Шукає за назвою, кодом і категорією товару.${extra} Кілька слів — у будь-якому порядку: «двигун кроковий» знайде «Кроковий двигун». Код можна вводити без дефіса: 05116 знайде 05-116. Великі й малі літери не розрізняються, як і кирилиця з латиницею в кодах і розмірах: М8 і M8, СО2 і CO2, 120х120 і 120x120.</div>
          <div class="help-term-desc">* — будь-які символи, зокрема пробіли: шків*19 знайде назви, де після «шків» десь є «19». ? — рівно один символ.</div>
          <div class="help-term-desc">Якщо точних збігів немає, пошук пробує той самий запит в іншій розкладці клавіатури, а потім схожі слова (одна помилка, у довгих словах — дві; коди й числа не розмиваються). Що саме показано, пише рядок над результатами.</div>
          <div class="help-term-desc">Клік по заголовку «Наявність» сортує результати по колу: спершу «Готово до відправки», спершу «Немає в наявності», знову від кращого збігу.</div>
        </div>`;

module.exports = { menuRow, helpMenuHtml, aboutPanelHtml, creditsPanelHtml, searchHelpHtml, writeLogos };
