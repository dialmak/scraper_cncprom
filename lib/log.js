// Логи прогонів: scrape.jsonl (скрапер) і map.jsonl (збірка сторінок).
//
// logEvent пише одну подію — самостійний JSON-об'єкт на рядок (JSONL). Цілісний
// JSON-документ дописувати не можна: його треба закривати дужкою, а скрапер
// пише в лог по рядку протягом трьох годин і може обірватись посеред ночі.
// JSONL дописується так само, як текст, а читається як дані — з нього
// lib/runlog.js будує таблиці «Лог скрапінгу» й «Лог збірки».
//
// Час зберігається в ISO UTC і форматується лише при показі (lib/time.js,
// київський пояс), щоб формат не був зашитий у дані.
//
// У консоль іде звичайний людський рядок (eventText): у GitHub Actions за
// прогоном стежать очима, і JSON там нічого не додав би.
...
// Товари, яких стосується попередження чи помилка, — по рядку на товар.
const itemLines = items => (items && items.length
  ? items.map(i => `\n    · ${i.name ? i.name + ' — ' : ''}${i.url}`).join('') : '');

// Людський рядок для консолі. Формулювання ті самі, що були в текстових
// scrape.log і map.log: за прогоном у CI стежать по цих словах.
function eventText(rec) {
  ...
  switch (rec.ev) {
    ...
    // Збірка сторінок (build-maps.js → map.jsonl).
    case 'build-start': return `СТАРТ build-maps: категорій ${f.total}.`;
    case 'build-finish': return `ФІНІШ build-maps: оброблено категорій ${f.ok} з ${f.total}` +
      `${f.errors ? `, помилок ${f.errors}` : ''}, ${f.sec} сек.`;
    case 'warn': return `УВАГА: ${f.msg}${itemLines(f.items)}`;
    case 'error': return `ПОМИЛКА: ${f.msg}${itemLines(f.items)}`;
    default: return `${rec.ev}: ${JSON.stringify(f)}`;
  }
}

// Один JSON-об'єкт на рядок. Порядок ключів сталий (t, ev, id — далі своє),
// щоб файл було видно очима навіть без розбору. Помилка запису (зайнятий файл,
...
module.exports = { nowStr, logEvent, readEvents, eventText };
