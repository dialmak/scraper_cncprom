...
//
// Результат: <dataDir>/reports/<todayDate>.md
...
today.products.forEach(p => {
  ...
  if (before.availability !== p.availability) {
    prodsAvailChanged.push({ id: p.id, name: p.name, before: before.availability, after: p.availability });
  }
  if (before.categoryId !== p.categoryId) {
    prodsMoved.push({ id: p.id, name: p.name, before: catName(before.categoryId), after: catName(p.categoryId) });
  }
});
